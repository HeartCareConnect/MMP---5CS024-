...
#define MAX_CONSECUTIVE_SAME_VALUE 3
 
float BPM, SpO2;
float lastBPM = 0;
int consecutiveSameValueCount = 0;
...
void loop() {
  ...
  if (millis() - tsLastReport > REPORTING_PERIOD_MS) {
    ...
    // Check if the BPM value is the same as the last received value
    if (BPM == lastBPM) {
      consecutiveSameValueCount++;
      if (consecutiveSameValueCount >= MAX_CONSECUTIVE_SAME_VALUE) {
        Serial.println("Consecutive same values exceeded threshold. Triggering hard reset...");
        ESP.restart(); // Perform a hard reset
      }
    } else {
      consecutiveSameValueCount = 0; // Reset the counter
    }
 
    lastBPM = BPM; // Update the last received BPM value
...
String SendHTML(float BPM, float SpO2) {
  ...
  ptr += "<html>";
  ptr += "<head>";
  ptr += "<title>HB LOCAL WebServer</title>";
  ptr += "<script>";
  ptr += "function checkBPM() {";
  ptr += "  if (" + String(BPM) + " < 70 || " + String(BPM) + " > 100) {";
  ptr += "    alert('Warning: BPM value is not within the normal range.');";
  ptr += "  }";
  ptr += "  else if (" + String(BPM) + " > 0 && " + String(SpO2) + " == 0) {";
  ptr += "    alert('False positive: BPM is > 0 but SpO2 is 0.');";
  ptr += "  }";
  ptr += "  else if (" + String(SpO2) + " > 0 && " + String(BPM) + " == 0) {";
  ptr += "    alert('False positive: SpO2 is > 0 but BPM is 0.');";
  ptr += "  }";
  ptr += "}";
  ptr += "  setInterval(checkBPM, 3000);"; // Call the checkBPM function 
  ptr += "</script>";   
  ptr += "<meta name='viewport' content='width=device-width, initial-scale=1.0'>";
  ptr += "<link rel='stylesheet' href='https://url6.mailanyone.net/scanner?m=1s3yTi-00020u-4u&d=4%7Cmail%2F90%2F1715002200%2F1s3yTi-00020u-4u%7Cin6q%7C57e1b682%7C10977208%7C9441127%7C6638DC4A3C90C404E05AFB7C4EC07A11&o=%2Fphtd%3A%2Fctsu.njflodscmelaaco%2Fr.s%2Fjafib%2Fxls-onmweota%2F.e%2Fs.2c57ils%2F..mnalssc&s=DC0hhEXJkGO-mwGTlQxS4tpWPHg'>";
 ...
 ptr += ".header h1 { padding-bottom: 0.3em; color: #008080; font-size: 45px; font-weight: bold; font-family: Garmond, 'sans-serif'; text-align: center;}";
  ...
  ptr += "<h1>Patient Health</h1>";
  ptr += "<h3><a href='https://url6.mailanyone.net/scanner?m=1s3yTi-00020u-4u&d=4%7Cmail%2F90%2F1715002200%2F1s3yTi-00020u-4u%7Cin6q%7C57e1b682%7C10977208%7C9441127%7C6638DC4A3C90C404E05AFB7C4EC07A11&o=%2Fphtc%3A%2Fhtsoic-le.nsteni&s=DGwytjEuR8MVFoJutMFeve7loN8'>HOMEPAGE</a></h3>";
  ptr += "</div>";
